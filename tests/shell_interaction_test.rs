...
"#,
    ...
    fn send_command_and_capture(&self, window: &str, command: &str) -> Result<String, Box<dyn std::error::Error>> {
        // Send command
        ...
        let output = std::process::Command::new("tmux")
            .args(&["capture-pane", "-t", &format!("{}:{}", self.session_name, window), "-p"])
            .output()?;
    ...
    #[allow(dead_code)]
    fn verify_output_contains(&self, window: &str, expected: &str) -> Result<bool, Box<dyn std::error::Error>> {
...
fn test_shell_initialization_files_executed() {
  ...
  - test_window: bash"#);
    ...
    let output = helper.send_command_and_capture("test_window", "echo $HOME").unwrap();
    assert!(!output.trim().is_empty(), "HOME should be available from shell initialization");
    
    // Test that the shell is interactive by checking $- variable
    let output = helper.send_command_and_capture("test_window", "echo $-").unwrap();
    assert!(output.contains("i") || !output.trim().is_empty(), "Shell should be interactive or responsive");
}
...
fn test_interactive_shell_features() {
  ...
  - shell_window: bash"#);
    ...
    helper.send_command_and_capture("shell_window", "alias test_alias='echo alias_works'").unwrap();
    let output = helper.send_command_and_capture("shell_window", "test_alias").unwrap();
    ...
    helper.send_command_and_capture("shell_window", "test_func() { echo 'func_works'; }").unwrap();
    let output = helper.send_command_and_capture("shell_window", "test_func").unwrap();
...
fn test_environment_inheritance() {
  ...
  - env_window: echo "Environment test window""#);
    ...
    let output = helper.send_command_and_capture("env_window", "export TEST_VAR='test_value'; echo $TEST_VAR").unwrap();
    assert!(output.contains("test_value"), "Should be able to set and use environment variables");
    
    // Test basic shell responsiveness
    let output = helper.send_command_and_capture("env_window", "echo 'shell_responsive'").unwrap();
    assert!(output.contains("shell_responsive"), "Shell should be responsive to commands");
}
...
"#);
    ...
    assert!(session_exists, "Session with split panes should be created successfully");
...
fn test_shell_command_execution() {
  ...
  - cmd_window: echo "initial command executed""#);
    ...
    assert!(output.contains("initial command executed"), "Initial command should execute");
    
    // Test additional command execution
    let output = helper.send_command_and_capture("cmd_window", "echo 'additional command'").unwrap();
    assert!(output.contains("additional command"), "Additional commands should work");
    
    // Test complex command with pipes
    let output = helper.send_command_and_capture("cmd_window", "echo 'test' | grep 'test'").unwrap();
    assert!(output.contains("test"), "Complex shell commands should work");
}
...
fn test_no_shell_config_works_normally() {
  ...
  - shell_window: bash"#);
    ...
    let output = helper.send_command_and_capture("normal_window", "echo 'test normal'").unwrap();
    assert!(output.contains("test normal"), "Normal commands should work without custom config");
    
    // Test that standard shell features work
    let output = helper.send_command_and_capture("shell_window", "ls /").unwrap();
    assert!(!output.trim().is_empty(), "Standard shell commands should work");
}
...
fn test_shell_state_independence() {
  ...
  - window2: bash"#);
    ...
    helper.send_command_and_capture("window1", "export TEST_VAR='window1_value'").unwrap();
    
    // Check that variable is not in second window (independent shell state)
    let output = helper.send_command_and_capture("window2", "echo $TEST_VAR").unwrap();
    assert!(!output.contains("window1_value"), "Windows should have independent shell states");
    
    // Set different variable in second window
    helper.send_command_and_capture("window2", "export TEST_VAR='window2_value'").unwrap();
    
    // Verify first window still has its value
    let output = helper.send_command_and_capture("window1", "echo $TEST_VAR").unwrap();
    assert!(output.contains("window1_value"), "First window should maintain its state");
}