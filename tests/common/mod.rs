...
impl TmuxTestSession {
    ...
    pub fn send_keys(&self, window_name: &str, keys: &str) -> Result<String, tmuxrs::error::TmuxrsError> {
...
impl Drop for TmuxTestSession {
    fn drop(&mut self) {
        ...
        if let Err(e) = TmuxCommand::kill_session(&self.session_name) {
            eprintln!("Warning: Failed to cleanup test session '{}': {}", self.session_name, e);
        }
    }
}
